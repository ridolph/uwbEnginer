    ...
    ui(new Ui::GraphicsWidget)
{
   ...
   /* tableHeader << "Tag ID/Label" << "X\n(m)" << "Y\n(m)" << "Z\n(m)" << "R95 \n(m)"
    ...
    ui->tagTable->setColumnCount(ColumnCount);
    ...
    ui->tagTable->setColumnWidth(ColumnHeartRate,70); //R95
    ui->tagTable->setColumnWidth(ColumnPacketLossRate,70); 
    ...
    ui->tagTable->setColumnWidth(ColumnRA0,50);
    ui->tagTable->setColumnWidth(ColumnRA1,50);
    ui->tagTable->setColumnWidth(ColumnRA2,50);
    ui->tagTable->setColumnWidth(ColumnRA3,50);
    ui->tagTable->setColumnWidth(ColumnRA4,50);
    ui->tagTable->setColumnWidth(ColumnRA5,50);
    ui->tagTable->setColumnWidth(ColumnRA6,50);
    ui->tagTable->setColumnWidth(ColumnRA7,50);
    ui->tagTable->setColumnWidth(ColumnRA8,50);
    ui->tagTable->setColumnWidth(ColumnRA9,50);
    ui->tagTable->setColumnWidth(ColumnRA10,50);
    ui->tagTable->setColumnWidth(ColumnRA11,50);
    ui->tagTable->setColumnWidth(ColumnRA12,50);
    ui->tagTable->setColumnWidth(ColumnRA13,50);
    ui->tagTable->setColumnWidth(ColumnRA14,50);
    ui->tagTable->setColumnWidth(ColumnRA15,50);
...
void GraphicsWidget::anchorTableClicked(int r, int c)
{
    ...
    if(c == ColumnID) //toggle label
    {
        ...
        emit updateAnchorShow(r, anc->show);
    }
}
...
{
    static double c_h = 0.8;
    static double c_s = 0.1;
    static double c_v = 0.3;
    Tag *tag;
    int tid = tagId;//& 0xf;
...
void GraphicsWidget::centerOnAnchors(void)
{
    ...
    pos1 = a1->a->pos() - QPointF(10.1, 10.1);
    pos2 = a1->a->pos() + QPointF(10.2, 10.2);
    pos3 = a1->a->pos() - QPointF(5.5, 5.5);
    QPolygonF p1 = QPolygonF() << pos1 << pos2 << pos3;
...
void GraphicsWidget::tagHeartRate(quint64 tagId, quint64 heartRate, quint64 power,quint64 plossRate)
{
    ...
    if(ridx != -1)
    {
        ui->tagTable->item(ridx, ColumnHeartRate)->setText(QString::number(heartRate));
        ui->tagTable->item(ridx, ColumnPower)->setText(QString::number(power));
		ui->tagTable->item(ridx, ColumnPacketLossRate)->setText(QString::number(plossRate));
    }
    else //add tag into tag table
    {
        tag->ridx = ui->tagTable->rowCount();
